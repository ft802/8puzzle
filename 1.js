var main = function() {
	...
	var a = 67; //сторона квадратной фишки
	var zazor = 3; //расстояние между соседними фишками
...
//создание псеводокласса---------------------------------------------
	...
	var mas_of_n = [n0,n1,n2,n3,n4,n5,n6,n7,n8];
...
//изменение размеров окна-------------------------------------------------
	...
	if(mobile){
		$('#astronaut').attr('src','nothing.png');
	}

	if (mobile) { $('#astronaut').fadeOut(); }
	...
	function imageresize() {
		...
		for (var i=0; i<9; i++){
			$("#"+mas_of_n[i].name).css("left",($(window).width()/2-(1.5*a+zazor)+(mas_of_n[i].x-1)*(a+zazor)));
			$("#"+mas_of_n[i].name).css("top",(90+(mas_of_n[i].y-1)*(a+zazor)));
		};
	}
...
//------------------------------------------------------------------  
	...
	var mix = function(){ 
		...
		for (var i=0; i<9; i++){
			...
			$("#"+mas_of_n[i].name).animate({
				left: String($(window).width()/2-(1.5*a+zazor)+(mas_of_n[i].x-1)*(a+zazor)),
				top: String(90+(mas_of_n[i].y-1)*(a+zazor))
			}, 500);
		};
	};
	...
	var moving = function(i) {
		...
		|| mas_of_n[0].y === mas_of_n[i].y && Math.abs(mas_of_n[0].x-mas_of_n[i].x)===1){
			...
			$("#"+mas_of_n[i].name).animate({
				left: String($(window).width()/2-(1.5*a+zazor)+(mas_of_n[0].x-1)*(a+zazor)),
				top: String(90+(mas_of_n[0].y-1)*(a+zazor))
			}, 500);
